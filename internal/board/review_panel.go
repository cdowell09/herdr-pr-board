...
func (m Model) openReviewPanel() (tea.Model, tea.Cmd) {
	...
	return m, tea.Batch(m.reviewHistoryCmd(pr.URL), reviewTick(pr.URL, m.reviewGeneration))
}
...
func (m Model) updateReview(message tea.Msg) (Model, tea.Cmd, bool) {
	...
	case reviewTickMsg:
		if m.reviewPanel != nil && m.reviewPanel.pr.URL == msg.url && msg.generation == m.reviewGeneration && m.reviews != nil {
			return m, tea.Batch(m.reviewHistoryCmd(msg.url), reviewTick(msg.url, msg.generation)), true
		}
		return m, nil, true
	case reviewDoneMsg:
...
func (m Model) reviewViewport() ([]string, int) {
	help := strings.Split(ansi.Wrap("n run · N explicit rerun · j/k scroll · o open · Esc back · q quit", max(1, m.width), ""), "\n")
