...
func TestTitleBarShowsTheVersionAtTheWideAndMediumTiers(t *testing.T) {
	...
	for _, tc := range cases {
		...
		refreshing := layoutModel(t, tc.width).WithVersion("0.6.0")
		refreshing.loading = true
		status := stripANSI(strings.Split(refreshing.View(), "\n")[0])
		if !strings.Contains(status, "refreshing") {
			t.Fatalf("width %d: title %q lost the status suffix", tc.width, status)
		}
	}
}
